...
from .models import HorarioDeParada
...
from django.http import HttpResponseBadRequest  # Importa HttpResponseBadRequest
...
########################
@api_view(['GET'])
def obtener_proxima_ruta(request, paradero_id, hora_actual=None):
    try:
        if hora_actual is None:
            # Si no se proporciona una hora actual, usa "00:00:00" como valor por defecto
            hora_actual = time(0, 0, 0)

        # Busca los horarios de llegada que son posteriores a la hora actual para esa parada
        horarios = HorarioDeParada.objects.filter(
            parada_de_autobus_id=paradero_id,
            hora_de_llegada__gte=hora_actual
        ).order_by('hora_de_llegada')

        if horarios:
            # Encuentra la parada de autobús correspondiente
            parada_de_autobus = horarios[0].parada_de_autobus
            # Construye una lista de horarios
            lista_de_horarios = [
                {
                    "dia_de_la_semana": horario.dia_de_la_semana,
                    "hora_de_llegada": horario.hora_de_llegada.strftime('%H:%M'),
                    "hora_de_salida": horario.hora_de_salida.strftime('%H:%M')
                }
                for horario in horarios
            ]

            # Encuentra la próxima ruta
            proxima_ruta = horarios[0].ruta_de_autobus

            return Response(
                {
                    "parada_de_autobus": parada_de_autobus.nombre,
                    "horarios": lista_de_horarios,
                    "proxima_ruta": proxima_ruta.nombre
                },
                status=status.HTTP_200_OK
            )
        else:
            return Response(
                {"message": "No se encontró información de próximos horarios para esta parada"},
                status=status.HTTP_404_NOT_FOUND
            )
    except ParadaDeAutobus.DoesNotExist:
        return Response(
            {"message": "No se encontró información de la parada de autobús"},
            status=status.HTTP_404_NOT_FOUND
        )
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if CustomUser.objects.filter(username=username).exists():
                ...
                # Autentica al usuario utilizando el backend ModelBackend
